...
import { ED_HEADING, ED_TEXT, ED_FAQ_BORDER, ED_CTA } from './theme';

/**
 * Flat grey FAQ bars, matching the reference layout and the accordion already
 * used on /ED-providers.
 *
 ...
 */
export function EdFaqList({ items }: { items: { q: string; a: string }[] }) {
  ...
  return (
    <div className="space-y-2.5">
      {items.map((item, index) => {
        ...
        return (
          <div key={item.q} style={{ border: `1px solid ${ED_FAQ_BORDER}`, borderRadius: 4 }}>
            <h3>
              <button
                type="button"
                ...
                id={`ed-faq-button-${index}`}
                className="flex w-full items-center justify-between gap-3 text-left"
                style={{ padding: '8px 16px', minHeight: 48, backgroundColor: '#fff', color: ED_TEXT, fontSize: 16 }}
              >
                <span>{item.q}</span>
            ...
            <div
              ...
              role="region"
              aria-labelledby={`ed-faq-button-${index}`}
              hidden={!isOpen}
              className="bg-white leading-relaxed"
              style={{ padding: '0 16px 12px', color: ED_TEXT, fontSize: 16 }}
            >
              {item.a}
            </div>
          </div>
