...
import { ED_CTA, ED_ACCENT } from './theme';
...
export function EdTopThreeFlashcard({
  providers,
  vertical = DEFAULT_VERTICAL,
}: {
  providers: EdHubProvider[];
  /** Tracking vertical for this page — every ED hub reuses this strip. */
  vertical?: string;
}) {
  return (
    <div>
      <div className="grid items-stretch gap-4 sm:grid-cols-3">
        {providers.map((provider) => {
          ...
          return (
            <div
              key={provider.slug}
              /* The top card's lift is tinted with the palette's CTA colour, so
                 both shadows resolve through the theme variables. */
              ...
              style={{
                ...
                border: isTop ? `2px solid ${ED_CTA}` : '1px solid #e5e7eb',
                transform: isTop ? 'translateY(-6px)' : undefined,
              }}
            >
              {/* Whole-card affiliate link. Sits under the CTA (z-index) so the
                  button keeps its own tracking; a click anywhere else on the
                  card still opens the provider. */}
              <OutboundLink
              ...
              <div className="flex flex-1 items-start justify-between gap-4">
                {/* Left: logo above the one-line differentiator */}
                ...
                {/* Right: score + stars above the CTA */}
                <div className="flex flex-none flex-col items-end gap-2">
                  <div className="flex items-center gap-1.5">
                    <EdStars rating={provider.rating} size={13} />
                    <span
                  ...
                  <OutboundLink
                    ...
                    style={{ backgroundColor: ED_CTA, textDecoration: 'none' }}
                  >
                    Visit Site
                    <svg
                      width="14"
