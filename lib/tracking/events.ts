export function track(eventName: string, payload: any): void {
  ...
  try {
    ...
    if (w._tfa && typeof w._tfa.push === 'function') {
      w._tfa.push({ notify: 'event', name: eventName, ...payload });
    }

    if (w.uetq && typeof w.uetq.push === 'function') {
      w.uetq.push('event', eventName, payload);
    }
...
// Fired when a user clicks a "Check Price" CTA
export function trackCheckPrice(payload: any): void {
  track('check_price_clicked', payload);
}
...
// Fired when a user clicks a Medvi-specific CTA
export function trackMedviReview(payload: any): void {
  track('medvi_cta_clicked', payload);
}

// Fired when a user interacts with the success stories slider
export function trackSlider(payload: any): void {
  track('success_stories_slider_clicked', payload);
}
